...
cv2.imwrite('./testdata/1a_gray.png', gray)
imga_gray = cv2.imread('./testdata/1a_gray.png')
...
jbf_3 = JBF.joint_bilateral_filter(img_rgb, imga_gray).astype(np.uint8) # Bilateral filter
jbf_1 = JBF.joint_bilateral_filter(img_rgb, gray).astype(np.uint8)      # Joint Bilateral filter
